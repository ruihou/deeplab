...
def flip_dim(image_list, prob=0.5):
  ...
  Args:
    image_list: A list of `Tensors` with the same number of dimensions.
  ...
  def flip():
    ...
    for tensor in image_list:
      flipped.append(tf.image.flip_left_right(tensor))
    return flipped

  with tf.control_dependencies(asserts):
    is_flipped = tf.less_equal(random_value, prob)
    outputs = tf.cond(is_flipped, flip, lambda: image_list)
...
def central_crop(image_list, crop_height, crop_width):
  """Central crops the give list of images.
